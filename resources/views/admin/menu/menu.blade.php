...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 py-4">
            ...
            <div class="w-full mb-1">
                ...
                <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                    <div class="flex items-center mb-4 sm:mb-0">
                        <form class="sm:pr-3" action="{{ route('menu') }}" method="GET">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                @forelse ($menus as $menu)
                                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                        ...
                                        <td class="p-2">
                                            @if ($menu->gambar)
                                                <img src="{{ asset($menu->gambar) }}" alt="Gambar {{ $menu->menu }}" width="100"
                                                    height="auto">
                                            @else
                                        ...
                                        <td class="p-4 space-x-2 whitespace-nowrap">
                                            <button
                                                onclick="update_menus('{{$menu->id}}', '{{$menu->menu}}', '{{$menu->harga}}', '{{$menu->stok}}', '{{$menu->deskripsi}}', {{$menu->kategori_id}} )"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center text-white p-4">Data tidak tersedia</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ $menus->previousPageUrl() ?? '#' }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg 
                   bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 
                   dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800
                   {{ $menus->onFirstPage() ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
                ...
                <a href="{{ $menus->nextPageUrl() ?? '#' }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg 
                   bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 
                   dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800
                   {{ $menus->hasMorePages() ? '' : 'opacity-50 cursor-not-allowed pointer-events-none' }}">
                    Selanjutnya
        ...
        <div id="drawer-update-product-default"
            ...
            <button type="button" data-drawer-dismiss="drawer-update-product" aria-controls="drawer-update-product"
                class="updateProductButton text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <form id="updateMenuForm" action="{{route('updateMenu')}}" method="post" enctype="multipart/form-data">
                @csrf
                {{-- @method('PUT') --}}
                <input type="text" name="id" hidden id="update-menu-id">
                <div class="space-y-4">
                    <div>
                        <label for="update-menu" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                            Menu</label>
                        <input type="text" name="menu" id="update-menu"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            required>
                    </div>
                    <div>
                        <label for="update-harga"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Harga</label>
                        <input type="number" name="harga" id="update-harga"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            required>
                    </div>

                    <div>
                        <label for="update-stok"
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="update-deskripsi" rows="4"
                    ...
                    <div class="mb-6">
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            for="update-gambar">Upload Gambar</label>
                        <input name="gambar" id="update-gambar" type="file"
        ...
        <div id="drawer-delete-product-default"
            ...
            @foreach ($menus as $menu)
                <form id="deleteMenuForm" method="POST" action="{{ route('deleteMenu', $menu->id) }}">
            @endforeach
                @csrf
                @method('DELETE')
                <button type="submit"
        ...
        <div id="drawer-create-product-default"
            ...
            <form action="{{ route('postMenu') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="menu" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                            Menu</label>
                    ...
                    <div class="mb-6">
                        ...
                        <div id="preview-container" class="hidden">
                            <img id="gambar-preview" src="#" alt="Preview Gambar" class="w-32 h-32">
                        </div>
                    </div>
    ...
        @endif
    <script>
        function update_menus(id, menu, harga, stok, deskripsi, kategori_id) {
            console.log("Kategori ID:", kategori_id);
            document.getElementById("update-menu-id").value = id;
            document.getElementById("update-menu").value = menu;
            ...
            // document.getElementById("updateKategori").value = kategori_id;
            // document.getElementById("nilai-awal").value = deskripsi;
            let kategoriSelect = document.getElementById("updateKategori");
            if (kategoriSelect) {
                kategoriSelect.value = kategori_id || ""; // Jika undefined, set ke string kosong
            } else {
                console.warn("Element updateKategori tidak ditemukan!");
            }

            // Jika nilai awal digunakan untuk kategori, seharusnya pakai kategori_id
            let nilaiAwal = document.getElementById("nilai-awal");
            if (nilaiAwal) {
                nilaiAwal.value = kategori_id || "";
            }
        }

        function hapusMenu(id) {
            console.log("Deleting menu ID:", id);
            document.getElementById("deleteMenuForm").action = `/menu/${id}`;
        }
    </script>
@endsection